...
export default function ViscontiConnectionMilestoneEditor({ practice }) {
  ...
  const save = async () => {
    ...
    setMessage('');
    try {
      // The API expects practice fields at the top level, not inside a payload object.
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-5 flex flex-col gap-3 border-b border-slate-100 pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-base font-semibold text-slate-900">Milestone e iter autorizzativo</h2>
          <p className="mt-1 max-w-3xl text-xs leading-5 text-slate-500">
            Registra solo eventi verificati. Il livello AT/AAT identifica il tipo di connessione; le scadenze procedurali vanno collegate alle date effettive della pratica.
          </p>
        </div>
        <button type="button" onClick={save} disabled={saving} className="shrink-0 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">
      ...
      <div className="mb-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <div className="mt-1 text-xs text-slate-500">Dati chiave per controllare l'avvio e l'avanzamento dell'iter.</div>
          </div>
          <div className="text-[10px] font-medium text-slate-400">Pratica: {practice.practiceCode || practice.practice_code || '—'}</div>
        </div>

        <div className="grid min-w-0 gap-3 md:grid-cols-3">
          <label className="min-w-0 block">
            <span className="mb-1.5 block text-xs font-semibold text-slate-600">Livello di tensione</span>
            <select value={values.voltage_level || ''} onChange={(e) => set('voltage_level', e.target.value)} className="block w-full min-w-0 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900">
          ...
          <label className="min-w-0 block">
            ...
            <select value={values.authorization_status || 'not_started'} onChange={(e) => set('authorization_status', e.target.value)} className="block w-full min-w-0 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900">
          ...
          <label className="min-w-0 block">
            ...
            <input value={values.authorization_outcome || ''} onChange={(e) => set('authorization_outcome', e.target.value)} placeholder="es. PAS perfezionato / autorizzato" className="block w-full min-w-0 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900" />
          </label>
        </div>
      </div>
      ...
      <div>
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-xs text-slate-500">Queste date alimentano il controllo della pratica e il workflow operativo.</div>
          </div>
          <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-500">{FIELDS.filter(([key]) => values[key]).length}/{FIELDS.length} registrate</span>
        </div>

        <div className="grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {FIELDS.map(([key, label]) => (
            <label key={key} className="min-w-0 block rounded-xl border border-slate-200 bg-white p-3">
              <span className="mb-1.5 block text-xs font-semibold text-slate-600">{label}</span>
              <input type="date" value={values[key] || ''} onChange={(e) => set(key, e.target.value)} className="block w-full min-w-0 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 outline-none focus:border-slate-400" />
            </label>
          ))}
        </div>
      </div>
