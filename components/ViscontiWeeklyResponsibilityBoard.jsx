...
function personMatches(name, person) { return norm(name) === norm(person); }
...
export default function ViscontiWeeklyResponsibilityBoard({ tasks = [], members = [] }) {
  const [area, setArea] = useState("Tutte");
  const names = useMemo(() => members.map(m => m.display_name || m.name).filter(Boolean), [members]);
  const visible = useMemo(() => {
    const base = tasks.filter(t => t.workflow_status !== "cancelled");
    if (area === "Tutte") return base;
    return base.filter(t => inferArea(t) === area || (area === "Connessioni" && t.connection_practice_id));
  }, [tasks, area]);
  const byPerson = useMemo(() => REVIEW_TEAM.map(person => ({ person, tasks: visible.filter(t => personMatches(t.assignee_name, person) || personMatches(names.find(n => personMatches(n, person)), t.assignee_name)) })), [visible, names]);
  const unassigned = visible.filter(t => !t.assignee_person_id && !t.assignee_name);
  const areaCounts = AREA_RULES.map(r => ({ ...r, count: tasks.filter(t => r.people.some(p => personMatches(t.assignee_name, p))).length }));
  ...
  return <section className="wm-week-board">
    ...
    <div className="wm-week-card">
      <div className="wm-week-head"><div><div className="wm-kicker">Agenda operativa di Vincenzo</div><div className="wm-title">Questa settimana: cosa deve succedere</div><div className="wm-sub">Il pannello legge le attività reali. Non crea né riassegna automaticamente i task.</div></div><div className="wm-filters">{["Tutte", ...AREA_RULES.map(r => r.area)].map(x => <button type="button" className={`wm-filter ${area===x ? "active" : ""}`} onClick={() => setArea(x)} key={x}>{x}</button>)}</div></div>
      ...
      <div className="wm-note"><b>Regola del lunedì:</b> verificare prima il lavoro precedente, poi decidere per ogni persona: chiudere, continuare, sbloccare/attendere un esterno oppure assegnare un nuovo incarico.</div>
    </div>
  </section>;
}
