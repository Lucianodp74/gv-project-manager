...
function nextAction(row, waiting, rejected) {
  if (rejected > 0) return "Verificare il riscontro Terna respinto e aggiornare la fase.";
  if (waiting > 0) return row.next_terna_confirmation ? `Attendere: ${row.next_terna_confirmation}` : "Attendere la conferma / validazione Terna.";
  if (row.overdue_deadlines > 0) return "Gestire subito la scadenza Terna scaduta.";
    ...
    <section className="ct-shell">
      ...
      <div className="ct-card">
        ...
        <div className="ct-actions">
          ...
          <div className={`ct-pill`}><span style={{fontWeight:800}}>Conferme Terna: {waiting}</span>{rejected > 0 && <span style={{marginLeft:6}}>· {rejected} da verificare</span>}</div>
