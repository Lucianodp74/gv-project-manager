...
export default function ViscontiNewProject({ members = [] }) {
  ...
  async function submit(event) {
    ...
    } catch (err) { setError(err.message || "Creazione non riuscita."); setSaving(false); }
  }
  const direct = !form.requires_spv;
  return <>
    <style>{`.np-btn{border:1px solid #172b4d;background:#172b4d;color:#fff;border-radius:9px;padding:9px 13px;font-size:11px;font-weight:800;cursor:pointer}.np-overlay{position:fixed;inset:0;background:rgba(15,23,42,.34);display:flex;align-items:center;justify-content:center;padding:20px;z-index:50}.np-modal{width:min(680px,100%);background:#fff;border-radius:16px;box-shadow:0 20px 60px rgba(15,23,42,.22);padding:22px}.np-head{display:flex;justify-content:space-between;gap:15px;align-items:flex-start}.np-title{font-size:20px;font-weight:850;color:#172033}.np-sub{font-size:11px;color:#737c8c;margin-top:4px}.np-close{border:0;background:#f3f5f7;border-radius:8px;width:32px;height:32px;cursor:pointer}.np-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:20px}.np-field{display:flex;flex-direction:column;gap:5px}.np-field.full{grid-column:1/-1}.np-label{font-size:10px;font-weight:800;color:#697386}.np-input,.np-select,.np-textarea{border:1px solid #dfe3e9;border-radius:9px;padding:10px;font:inherit;font-size:11px;outline:none}.np-textarea{min-height:78px;resize:vertical}.np-mode{border:1px solid #dfe3e9;border-radius:11px;padding:12px;display:flex;gap:10px;align-items:flex-start;cursor:pointer}.np-mode.active{border-color:#172b4d;background:#f7f9fc}.np-mode input{margin-top:2px}.np-mode b{font-size:11px;color:#172033}.np-mode span{display:block;font-size:9px;color:#737c8c;margin-top:3px}.np-footer{display:flex;justify-content:flex-end;gap:8px;margin-top:18px}.np-cancel{border:1px solid #dfe3e9;background:#fff;border-radius:9px;padding:9px 13px;font-size:11px;font-weight:750;cursor:pointer}.np-error{margin-top:12px;padding:9px;border-radius:8px;background:#fff0ef;color:#b43a34;font-size:10px}@media(max-width:620px){.np-grid{grid-template-columns:1fr}.np-field.full{grid-column:auto}}`}</style>
    ...
    {open && <div className="np-overlay" role="dialog" aria-modal="true" onMouseDown={(e) => { if (e.target === e.currentTarget && !saving) setOpen(false); }}>
      <form className="np-modal" onSubmit={submit}>
        <div className="np-head"><div><div className="np-title">Nuovo progetto</div><div className="np-sub">Progettazione tecnica a cura di Gruppo Visconti Servizi.</div></div><button type="button" className="np-close" onClick={() => !saving && setOpen(false)}>×</button></div>
