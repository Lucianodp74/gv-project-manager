...
export default function ViscontiDirectorControlTower({ data = {} }) {
  ...
  const exceptions = projects
    .filter((p) => p.risk_level !== "normal" || Number(p.blocked_tasks || 0) > 0 || Number(p.pending_decisions || 0) > 0 || Number(p.open_authority_items || 0) > 0 || Number(p.waiting_terna_confirmations || 0) > 0 || Number(p.rejected_terna_confirmations || 0))
    .slice(0, 8);
  const focus = [...open]
