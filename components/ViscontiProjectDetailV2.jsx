    ...
    <section className="pd-main">
      ...
      {tab==="overview"&&<>
        ...
        <div className="pd-grid"><div>
          ...
          <section className="pd-card"><div className="pd-title2">Da fare ora</div>{openTasks.slice(0,6).map(t=><div className="pd-row" key={t.id}><div><b>{t.title}</b><div className="pd-muted">{t.assignee_name || "Non assegnata"}{t.next_action ? ` · ${t.next_action}` : ""}</div></div><div>{statusLabel[t.workflow_status] || t.workflow_status}</div><div>{fmt(t.due_date)}</div><div><Badge tone={attentionTone(t.attention_state)}>{t.attention_state || "normal"}</Badge></div></div>)}{!openTasks.length&&<div className="pd-empty">Nessuna attività aperta.</div>}</section>
      ...
      {tab==="tasks"&&<section className="pd-card"><div className="pd-title2">Attività del progetto</div>{tasks.map(t=><div className="pd-row" key={t.id}><div><b>{t.title}</b><div className="pd-muted">{t.assignee_name || "Non assegnata"}{t.next_action ? ` · ${t.next_action}` : ""}</div></div><div>{statusLabel[t.workflow_status] || t.workflow_status}</div><div>{fmt(t.due_date)}</div><div><Badge tone={attentionTone(t.attention_state)}>{t.attention_state || "normal"}</Badge></div></div>)}{!tasks.length&&<div className="pd-empty">Nessuna attività presente.</div>}</section>}
