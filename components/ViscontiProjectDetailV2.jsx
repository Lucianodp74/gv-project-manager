...
export default function ViscontiProjectDetailV2() {
  ...
  return <main className="pd-shell"><style>{`.pd-shell{min-height:100vh;background:#f6f7f9;color:#172033;font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif}.pd-top{background:#fff;border-bottom:1px solid #e7e9ee;padding:14px 34px;display:flex;justify-content:space-between;align-items:center}.pd-brand{font-weight:800}.pd-back{display:block;color:#687181;text-decoration:none;font-size:11px;margin-bottom:4px}.pd-main{max-width:1380px;margin:auto;padding:30px 34px 50px}.pd-head{display:flex;justify-content:space-between;gap:20px;align-items:flex-start}.pd-kicker{font-size:10px;text-transform:uppercase;letter-spacing:.12em;color:#8a92a1;font-weight:800}.pd-title{font-size:30px;letter-spacing:-.04em;margin:5px 0}.pd-sub{font-size:13px;color:#737c8c}.pd-actions{display:flex;gap:8px}.pd-btn{border:1px solid #dfe3e9;background:#fff;border-radius:9px;padding:9px 12px;font-size:11px;font-weight:750;text-decoration:none;color:#172033}.pd-primary{background:#172b4d;color:#fff;border-color:#172b4d}.pd-grid{display:grid;grid-template-columns:1.5fr .75fr;gap:18px;margin-top:20px}.pd-card{background:#fff;border:1px solid #e7e9ee;border-radius:14px;padding:19px;box-shadow:0 2px 10px rgba(20,28,45,.03)}.pd-card+.pd-card{margin-top:18px}.pd-title2{font-size:14px;font-weight:800;margin-bottom:14px}.pd-kpis{display:grid;grid-template-columns:repeat(5,1fr);gap:9px}.pd-kpi{background:#fff;border:1px solid #e7e9ee;border-radius:12px;padding:13px}.pd-kpi small{display:block;color:#8a92a1;font-size:9px;text-transform:uppercase}.pd-kpi b{display:block;font-size:21px;margin-top:5px}.pd-meta{display:grid;grid-template-columns:repeat(4,1fr);gap:9px}.pd-box{background:#f8f9fb;border-radius:10px;padding:11px}.pd-box small{display:block;color:#8a92a1;text-transform:uppercase;font-size:9px}.pd-box strong{display:block;font-size:12px;margin-top:5px}.pd-task{display:grid;grid-template-columns:1.6fr .9fr .75fr .65fr;gap:10px;align-items:center;text-decoration:none;color:inherit;border-top:1px solid #eef0f3;padding:12px 0;font-size:11px}.pd-task:first-of-type{border-top:0}.pd-task:hover{background:#fafbfc}.pd-muted{color:#7c8493}.pd-badge{display:inline-flex;border-radius:999px;padding:4px 7px;font-size:9px;font-weight:800;white-space:nowrap}.pd-neutral{background:#f0f2f5;color:#626b7a}.pd-green{background:#eaf8f1;color:#18794e}.pd-amber{background:#fff5df;color:#996400}.pd-red{background:#fff0ef;color:#b43a34}.pd-blue{background:#edf3ff;color:#3d61ad}.pd-conn{border:1px solid #e8ebf0;border-radius:11px;padding:13px;margin-top:9px;background:#fff;text-decoration:none;color:inherit;display:block}.pd-conn:hover{border-color:#b9c5da}.pd-conn strong{font-size:12px}.pd-conn small{display:block;color:#7c8493;margin-top:4px}.pd-alert{border:1px solid #f0dcb4;background:#fff9ed;border-radius:11px;padding:13px;font-size:11px}.pd-alert p{font-size:10px;color:#747d8b;line-height:1.45;margin:7px 0 0}.pd-empty{color:#808897;font-size:12px;padding:12px 0}.pd-loading,.pd-error{max-width:800px;margin:80px auto;padding:30px;background:#fff;border:1px solid #e7e9ee;border-radius:14px}.pd-error a{display:block;margin-top:15px;color:#3d61ad;text-decoration:none;font-size:12px}@media(max-width:950px){.pd-grid{grid-template-columns:1fr}.pd-meta{grid-template-columns:1fr 1fr}.pd-kpis{grid-template-columns:repeat(3,1fr)}.pd-main{padding:22px 18px}}@media(max-width:620px){.pd-head{display:block}.pd-actions{margin-top:14px}.pd-kpis{grid-template-columns:1fr 1fr}.pd-task{grid-template-columns:1fr 1fr}.pd-task div:nth-child(4){grid-column:span 2}}`}</style>
    ...
    <section className="pd-main"><div className="pd-head"><div><div className="pd-kicker">Centro operativo del progetto</div><h1 className="pd-title">{project.name}</h1><p className="pd-sub">{stageLabel[project.status] || project.status || "—"} · {project.region || "—"} · {project.power_mw || 0} MW</p></div><Badge tone={blocked || overdue || waitingConfirmations ? "amber" : "green"}>{blocked ? `${blocked} blocchi` : overdue ? `${overdue} scadenze attività` : waitingConfirmations ? `${waitingConfirmations} conferme in attesa` : "In linea"}</Badge></div>
      ...
      <div className="pd-grid"><div><section className="pd-card"><div className="pd-title2">Quadro del progetto</div><div className="pd-meta"><div className="pd-box"><small>Fase</small><strong>{stageLabel[project.status] || project.status || "—"}</strong></div><div className="pd-box"><small>Codice</small><strong>{project.project_code || "—"}</strong></div><div className="pd-box"><small>Responsabile</small><strong>{memberName(project.responsible_id)}</strong></div><div className="pd-box"><small>Regione</small><strong>{project.region || "—"}</strong></div></div></section>
      ...
      <aside><section className="pd-card"><div className="pd-title2">Controllo</div><div className="pd-alert"><b>{blocked ? "Ci sono attività bloccate." : overdue ? "Ci sono attività scadute." : waitingConfirmations ? "In attesa di conferme esterne." : "Nessuna criticità rilevata."}</b><p>La scheda mostra solo ciò che serve per capire cosa fare, chi è responsabile e dove una connessione richiede attenzione.</p></div></section>
