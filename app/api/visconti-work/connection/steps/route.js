...
async function shiftSteps(practiceId, startOrder) {
  const response = await fetch(`${SUPABASE_URL}/rest/v1/connection_steps?select=id,sort_order&practice_id=eq.${encodeURIComponent(practiceId)}&sort_order=gte.${startOrder}&order=sort_order.desc,id.desc`, { headers: headers() });
  if (!response.ok) throw new Error('Impossibile leggere le fasi da spostare');
  const rows = await response.json();
  for (const row of rows) {
    const patch = await fetch(`${SUPABASE_URL}/rest/v1/connection_steps?id=eq.${encodeURIComponent(row.id)}`, { method: 'PATCH', headers: headers({ Prefer: 'return=minimal' }), body: JSON.stringify({ sort_order: Number(row.sort_order) + 1 }) });
...
export async function POST(request) {
  ...
  try {
    ...
    if (anchorId && ['before', 'after'].includes(position)) {
      const anchorRes = await fetch(`${SUPABASE_URL}/rest/v1/connection_steps?select=id,sort_order&practice_id=eq.${encodeURIComponent(practiceId)}&id=eq.${encodeURIComponent(anchorId)}&limit=1`, { headers: headers() });
...
export async function DELETE(request) {
  ...
  try {
    ...
    const response = await fetch(`${SUPABASE_URL}/rest/v1/connection_steps?id=eq.${encodeURIComponent(id)}`, { method: 'DELETE', headers: headers({ Prefer: 'return=representation' }) });
    const data = await response.json().catch(() => []);
