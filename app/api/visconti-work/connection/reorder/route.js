...
export async function POST(request) {
  ...
  try {
    ...
    const response = await fetch(`${SUPABASE_URL}/rest/v1/connection_steps?select=id,practice_id&practice_id=eq.${encodeURIComponent(practiceId)}&order=sort_order.asc,id.asc`, { headers: headers() });
    ...
    // Temporary unique-ish positions prevent collisions while rows are reordered.
