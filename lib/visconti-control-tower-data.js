...
async function get(path) {
  ...
  const response = await fetch(`${SUPABASE_URL}/rest/v1/${path}`, { headers, next: { revalidate: 15, tags: ["visconti-control-tower"] } });
...
export async function getViscontiControlTowerData() {
  const [projects, tasks, connections, summaries] = await Promise.all([
    safe("projects?select=id,name,region,status,power_mw,project_code,project_type,responsible_id,go_no_go_status,go_no_go_date,spv_name,spv_status,connection_holder,connection_transfer_status,connection_transfer_request_date,connection_transfer_completed_date&status=neq.archived&order=name.asc"),
    safe("visconti_task_board?select=id,project_id,title,next_action,due_date,responsible_id,workflow_status,attention_state,blocker_reason,description,created_at,is_terna_confirmation_task,priority&workflow_status=neq.done&workflow_status=neq.cancelled&order=due_date.asc.nullslast,created_at.desc&limit=120", []),
    safe("connection_practices?select=project_id,authorization_status,authorization_outcome,authorization_start_at&status=in.(pending,active,accepted)&order=updated_at.desc&limit=300", []),
  ...
  return {
    projects: projects.map((p) => { const summary = summaryById.get(p.id) || {}; const projectTasks = tasksByProject.get(p.id) || []; const nextTask = projectTasks[0] || null; return { ...summary, ...p, project_id: p.id, project_name: p.name, project_status: p.status, project_stage: stageFromConnection(connectionByProject.get(p.id)) || p.status, responsible_name: p.responsible_id || "Non assegnato", open_tasks: summary.open_tasks ?? projectTasks.length, blockers: summary.blockers ?? projectTasks.filter((t) => t.workflow_status === "blocked" || t.attention_state === "blocked").length, overdue_tasks: summary.overdue_tasks ?? projectTasks.filter((t) => t.attention_state === "overdue").length, next_action: nextTask?.next_action || nextTask?.title || null, next_action_task_id: nextTask?.id || null, next_action_due_date: nextTask?.due_date || null, next_action_responsible_name: null, next_action_blocker: nextTask?.workflow_status === "blocked" ? (nextTask.blocker_reason || nextTask.description || null) : null, spv_attention: spvAttention(p) }; }),
