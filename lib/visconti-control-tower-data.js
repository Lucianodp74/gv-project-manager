...
const DEFAULT_SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.408iZrkj5i2Ikh0FL91N1a1AuDJFAAIehD0H9q6G9s";
...
async function safe(path, fallback = []) {
  try { return await get(path); } catch (error) { console.error(`Control Tower data: ${path}`, error); return fallback; }
}

function stageFromConnection(connection) {
...
function spvAttention(project) {
  if (project.go_no_go_status === "go" && project.connection_transfer_status !== "completed") {
    return project.spv_status === "created" ? "Voltura da completare" : "SPV da completare";
  }
  return null;
}
