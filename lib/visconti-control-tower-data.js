...
export async function getViscontiControlTowerData() {
  const [projects, tasks, members, connections, summaries] = await Promise.all([
    ...
    safe("visconti_task_board?select=id,project_id,title,next_action,due_date,responsible_id,workflow_status,attention_state,blocker_reason,description,created_at&workflow_status=neq.done&workflow_status=neq.cancelled&order=due_date.asc.nullslast,created_at.desc&limit=150"),
    safe("team_members?select=id,display_name&active=eq.true&order=display_name.asc&limit=200"),
    safe("connection_practices?select=project_id,authorization_status,authorization_outcome,authorization_start_at&order=updated_at.desc&limit=300"),
