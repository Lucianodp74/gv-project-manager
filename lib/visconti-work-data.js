...
export async function getViscontiWorkData({ includeArchived = false } = {}) {
  try {
    ...
    const visibleProjects = includeArchived ? projects : projects.filter((project) => project.project_status !== "archived");
...
export async function getViscontiConnectionsData() {
  try {
    ...
    const [members, rawPractices] = await Promise.all([
      supabaseGet("team_members?select=id,verification_status,source_label,source_url,source_verified_at"),
