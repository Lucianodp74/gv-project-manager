...
function operationalStage(connection) {
  ...
  if (connection.authorization_outcome) return "authorized";
  if (connection.authorization_start_at) return "authorization";
  return "connection";
}
...
export async function getViscontiWorkData() {
  try {
    const [projects, tasks, members, connections] = await Promise.all([
      ...
      supabaseGet("connection_workflow_overview?select=project_id,accepted_at,pto_validated_at,authorization_start_at,authorization_outcome&status=in.(pending,active,accepted)&limit=500"),
    ]);
