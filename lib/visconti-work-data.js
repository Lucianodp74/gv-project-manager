...
export async function getViscontiWorkData({ includeArchived = false } = {}) {
  ...
  const rawProjects = await supabaseGetSafe("projects?select=id,name,region,status,power_mw,project_code,responsible_id,go_no_go_status,go_no_go_date,spv_name,spv_status,connection_holder,connection_transfer_status,connection_transfer_request_date,connection_transfer_completed_date&order=name.asc", []);
  if (!rawProjects.length) return { projects: [], tasks: [], members: [], connected: true };
