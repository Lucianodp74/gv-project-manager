  ...
  try {
    ...
    return {
      projects: projects.map((project) => ({
        ...project,
        project_stage: project.project_status,
  ...
  try {
    ...
    return {
      connections: practices.map((p) => ({
        ...
        attention_state: p.overdue_deadlines > 0 || (p.next_deadline && p.next_deadline < isoToday())
          ? "overdue"
          : p.next_deadline && p.next_deadline <= isoToday(7) ? "soon" : "normal",
      })),
      connected: hasSupabase,
    };
  } catch (error) {
  ...
  try {
    ...
    return {
      ...
      deadlines: deadlines.map((deadline) => ({
        ...
        attention_state: deadline.status === "overdue" || deadline.due_date < isoToday() ? "overdue" : deadline.due_date <= isoToday(3) ? "urgent" : deadline.due_date <= isoToday(7) ? "soon" : "normal",
        source_task_id: taskByDeadline.get(deadline.id)?.id || null,
        source_task_status: taskByDeadline.get(deadline.id)?.workflow_status || null,
      })),
      steps: steps.map((step) => {
        const confirmationTask = taskByStep.get(step.id);
        return {
          ...step,
          responsible_name: step.responsible_name || memberName.get(step.responsible_id) || memberName.get(practice.responsible_id) || "Non assegnato",
          source_confirmation_task_id: confirmationTask?.id || null,
          source_confirmation_task_status: confirmationTask?.workflow_status || null,
          source_confirmation_task_attention: confirmationTask?.attention_state || null,
          source_confirmation_task_next_action: confirmationTask?.next_action || null,
        };
      }),
      members,
