...
export async function getViscontiWorkData({ includeArchived = false } = {}) {
  try {
    // Keep every query limited to columns that actually exist in the current Supabase schema.
    const [summary, rawProjects, tasks, members, connections] = await Promise.all([
      supabaseGet("project_operational_summary?select=*&order=overdue_deadlines.desc,project_id.asc"),
    ...
    return {
      ...
      tasks: visibleTasks, members, connected: hasSupabase,
    };
  } catch (error) {
