...
export async function getViscontiConnectionsData() {
  try {
    ...
    const [members, rawPractices] = await Promise.all([
      supabaseGetSafe("team_members?select=id,display_name&active=eq.true&order=display_name.asc&limit=200"),
      supabaseGetSafe("connection_practices?select=id,verification_status,source_label,source_url,source_verified_at"),
    ]);
    const memberName = new Map(members.map((member) => [member.id, member.display_name]));
...
export async function getViscontiConnectionData(practiceId = null, projectId = null) {
  try {
    ...
    const [projects, deadlines, steps, members, sourceTasks, controlTower, rawPractice] = await Promise.all([
      supabaseGetSafe(`projects?select=id,name&id=eq.${practice.project_id}&limit=1`),
      supabaseGetSafe(`connection_deadlines?select=*&practice_id=eq.${practice.id}&status=neq.cancelled&order=due_date.asc`),
      supabaseGetSafe(`connection_workflow_builder?select=*&practice_id=eq.${practice.id}&order=sort_order.asc`),
      supabaseGetSafe("team_members?select=id,display_name&active=eq.true&order=display_name.asc&limit=200"),
      supabaseGetSafe(`visconti_task_board?select=id,source_connection_deadline_id,source_connection_step_id,workflow_status,attention_state,next_action&connection_practice_id=eq.${practice.id}`),
      supabaseGetSafe(`terna_connection_control_tower?select=*&practice_id=eq.${practice.id}&limit=1`),
      supabaseGetSafe(`connection_practices?select=*&id=eq.${practice.id}&limit=1`),
    ]);
    const memberName = new Map(members.map((member) => [member.id, member.display_name]));
