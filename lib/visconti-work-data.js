...
const DEFAULT_SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imp5aW5kZHZ2Y25seXpha2VnZ3AiLCJyb2xlIjoiYW5vbiIsImlhdCI6MTc4ODM0OTQxMiwiZXhwIjoyMTAzOTI1NDEyfQ.408iZ2rkj5i2Ikh0FL91N1a1AuDJFAAIehD0H9q6G9s";
...
function spvAttention(project) {
  ...
  if (project.go_no_go_status === "go" && project.connection_transfer_status !== "completed") {
    if (project.spv_status !== "created") return "SPV da completare";
    return "Voltura da completare";
  }
  return null;
}
...
export async function getViscontiWorkData({ includeArchived = false } = {}) {
  try {
    const [summary, rawProjects, tasks, members, connections] = await Promise.all([
      supabaseGet("project_operational_summary?select=*&order=operational_attention.desc,project_name.asc"),
      supabaseGet("projects?select=id,name,region,status,power_mw,project_code,responsible_id,go_no_go_status,go_no_go_date,spv_name,spv_status,connection_holder,connection_transfer_status,connection_transfer_request_date,connection_transfer_completed_date&order=name.asc"),
      supabaseGet("visconti_task_board?select=*&workflow_status=neq.done&workflow_status=neq.cancelled&order=due_date.asc.nullslast,created_at.desc&limit=500"),
    ...
    const visibleProjectIds = new Set(visibleProjects.map((project) => project.project_id));
...
export async function getViscontiConnectionData(practiceId = null, projectId = null) {
  try {
    ...
    const practiceDetail = {
      ...practice,
      ...verification,
      request_date: verification.request_date || "",
      pto_received_date: verification.pto_received_date || "",
      pto_accepted_date: verification.pto_accepted_at || "",
      pto_validated_date: verification.pto_validated_at || "",
      iter_start_date: verification.authorization_start_at || "",
      start_works_validated_date: verification.start_works_validated_at || "",
      sharing_date: verification.sharing_date || "",
