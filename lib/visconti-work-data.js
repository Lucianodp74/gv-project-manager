...
export async function getViscontiConnectionData(practiceId = null, projectId = null) {
  try {
    ...
    const taskByDeadline = new Map(
      sourceTasks.filter(t => t.source_connection_deadline_id).map(t => [t.source_connection_deadline_id, t])
    );
    ...
    return {
      ...
      controlTower: controlTower.map(item => ({
        ...item,
        responsible_name: memberName.get(item.responsible_id) || "Non assegnato",
      })),
