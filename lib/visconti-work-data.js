...
export async function getViscontiConnectionData(practiceId = null, projectId = null) {
  try {
    ...
    if (!practices.length) return { practice: null, deadlines: [], steps: [], history: [], controlTower: [], members: [], connected: hasSupabase };
    const practice = practices[0];
    const [projects, deadlines, steps, members, sourceTasks, history, controlTower] = await Promise.all([
      ...
      supabaseGet(`connection_workflow_history?select=id,practice_id,step_id,action,changed_at,changed_by,changed_by_name,step_title,old_status,new_status,old_confirmation_status,new_confirmation_status,old_due_date,new_due_date,old_is_not_applicable,new_is_not_applicable&practice_id=eq.${practice.id}&order=changed_at.desc&limit=200`),
  ...
  } catch (error) { console.error("Visconti connection data load failed:", error); return { practice: null, deadlines: [], steps: [], history: [], controlTower: [], members: [], connected: false, error: "Dati connessione non disponibili" }; }
}
