...
export async function getViscontiConnectionsData() {
  try {
    ...
    const projects = projectIds.length
      ? await supabaseGet(`projects?select=id,name,region&id=in.(${projectIds.join(",")})`)
      : [];
    ...
    return {
      connections: practices.map(p => ({
        ...
        attention_state: p.next_deadline && p.next_deadline < isoToday() ? "overdue" : p.next_deadline && p.next_deadline <= isoToday(7) ? "soon" : "normal",
      })),
...
export async function getViscontiConnectionData(practiceId = null, projectId = null) {
  try {
    ...
    if (!practices.length) {
      return { practice: null, deadlines: [], steps: [], controlTower: [], connected: hasSupabase };
    }
    ...
    return {
      ...
      deadlines: deadlines.map(deadline => {
        const task = taskByDeadline.get(deadline.id);
        return {
          ...deadline,
          ...
          source_task_status: task?.workflow_status || null,
        };
      }),
    ...
    return { practice: null, deadlines: [], steps: [], controlTower: [], connected: false, error: "Dati connessione non disponibili" };
  }
}
