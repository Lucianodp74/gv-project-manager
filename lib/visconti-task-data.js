...
async function safe(path, fallback = []) {
  try { return await supabaseGet(path); } catch (error) { console.error(`Visconti task query failed: ${path}`, error); return fallback; }
}

export async function getViscontiTaskData() {
  if (!hasSupabase) return { tasks: [], projects: [], members: [], practices: [], deadlines: [], connected: false, error: "Supabase non configurato" };
  // Tasks are primary; auxiliary connection data must not blank the whole board.
  let tasks = [];
  let projects = [];
  try {
    ...
    console.error("Visconti task primary load failed:", error);
    return { tasks: [], projects: [], members: [], practices: [], deadlines: [], connected: false, error: "Attività non disponibili" };
  }
  const [members, practices, deadlines, workflowSteps] = await Promise.all([
    safe("team_members?select=id,display_name,role,active&active=eq.true&order=display_name.asc&limit=200"),
    safe("connection_practices?select=id,project_id,practice_code,status&status=in.(pending,active,accepted)&order=created_at.desc&limit=300"),
    safe("connection_deadlines?select=id,practice_id,title,due_date,status,responsible_id&order=due_date.asc.nullslast&limit=500"),
    safe("connection_workflow_builder?select=id,practice_id,title,confirmation_required,confirmation_status&limit=1000"),
  ]);
