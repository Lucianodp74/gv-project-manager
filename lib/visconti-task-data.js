...
export async function getViscontiTaskData() {
  try {
    const [tasks, projects, members, practices, deadlines] = await Promise.all([
      ...
      supabaseGet("connection_deadlines?select=id,practice_id,title,deadline_type,due_date,status,responsible_id&order=due_date.asc.nullslast&limit=500"),
    ]);
    ...
    return {
      tasks: tasks.map(t => {
        ...
        return {
          ...
          assignee_name: t.assignee_name || memberMap.get(t.assignee_person_id)?.display_name || "Non assegnata",
          connection_practice_code: practice?.practice_code || null,
          connection_deadline_title: deadline?.title || null,
          connection_deadline_type: deadline?.deadline_type || null,
